...
| `frequency` | `enum` | `daily`, `alternate`, `weekly`, etc. |
...
### The Sync Loop
1.  **Write**: Any user action (`complete`, `add`, `update`) updates the local Dexie store and sets `synced: false`.
2.  **Detection**: The `ClockworkContext` uses a `useLiveQuery` to monitor the database for any records where `synced === false`.
3.  **Manual Trigger**: If `hasUnsyncedChanges` is true, the UI displays a "Unsynced changes" warning.
